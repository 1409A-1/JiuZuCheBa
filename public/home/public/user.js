    ...
    jQuery.ajax({
        ...
        success: function (result) {
            ...
            for (var i = 0; i < num; i++) {
                ...
                html += "<li><h5><a>订单号：</a>" + order.contract_file_id + "</h5>" + "<div><img src='" + api_url + img + "'>" + "<div class='info'><a id='car_name'>" + car.honda + " " + car.class_name + "</a>" + "<div class='carIcon'><div>" + "<i class='icon icon_car1'></i>" + "<a>" + car.body_construction + "</a></div><div><i class='icon icon_car2'></i>" + "<a>" + car.gearbox + "</a></div><div><i class='icon icon_car3'></i>" + "<a>" + car.let_litre + "</a></div><div><i class='icon icon_car4'></i>" + "<a>乘坐" + car.seat_count + "人</a></div></div></div>" + "<ul><li class='time'><div>取车时间<br>" + time1 + "</div>" + "<div id='times'>还车时间<br>" + time2 + "</div></li>" + "<li class='cen'>总额：￥" + order.standard_rent + "</li>" + "<li class='state'>" + get_state(order.order_state) + "</a></li>" + "<li class='operation'>";

                if (TYPE == 1 || (order.order_state & 1) == 1) { //预约中订单 添加 取消按钮
                    var yusq = order.define_rent_deposit;
                    var old_rent = order.rent_deposit_pre_authorization_amount + order.rent_deposit_online_bank_payment_amount
                        + order.rent_deposit_pos_payment_amount + order.rent_deposit_cash_payment_amount + order.rent_deposit_cheque_payment_amount + order.rent_deposit_coupon_payment_amount + order.rent_deposit_score_payment_amount + order.rent_deposit_other_payment_amount;
                    if (old_rent < yusq) {
                        html += "<div><button class='to_pay' style='cursor:pointer;width: 73px;height: 25px;background-color: #f36a2d;border: none;color: #fff;border-radius: 4px; margin: 0 auto;position:absolute;top:49px;right:38px;' onclick='pay(" + order.order_id + ")' id='onlin_pay'>去付款</button><a href='/home/orderdetail?order_id=" + order.order_id + "'>订单详情</a><br>" + "<a onclick='cancelOrder(" + order.order_id + ")'>取消订单</a></div>" + "</li></ul></div></li>";
                    } else {
                ...
                $("#out_trade_no").val($.trim(order.contract_file_id));
                $("#subject").val(order.auto.autoclass.honda + order.auto.autoclass.class_name);
                $("#total_fee").val(order.define_rent_deposit);
                $(".shortOrder>.order3").html(html);

                //var yyusq = order.rent_deposit_pre_authorization_amount;
...
function pay(order_id) {
    ...
    var stop_time = $("#times").text();
    daysBetween(time, stop_time,order_id);
}
//| 求两个时间的天数差 日期格式为 YYYY-MM-dd
function daysBetween(time, stop_time, order_id) {
    ...
    var TwoMonth = stop_time.substring(9, stop_time.lastIndexOf('-'));
    var TwoDay = stop_time.substr(12,2);
    var TwoYear = stop_time.substring(4, stop_time.indexOf('-'));
